...
function renderizarTabla(alumnos) {
  ...
  if (alumnos.length === 0) {
    tbody.innerHTML = '<tr><td colspan="5" class="mensaje-vacio">No hay alumnos registrados.</td></tr>';
    return;
  }
    ...
    <tr data-id="${alumno.id}">
      ...
      <td>
        <button
          ...
          aria-label="Eliminar alumno ${alumno.nombre} ${alumno.apellido}"
        >
          🗑 Eliminar
        </button>
      </td>
  ...
  // Acá asigno el evento click a cada botón de eliminar recién renderizado
  tbody.querySelectorAll('.btn-eliminar').forEach(btn => {
    btn.addEventListener('click', () => eliminarAlumno(Number(btn.dataset.id)));
  });
}
...
// Acá envío una petición DELETE a la API para eliminar el alumno con el ID dado
