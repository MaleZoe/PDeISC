...
async function cargarConFetch() {
  ...
  actualizarExplicacion('Fetch es la API nativa del navegador. Enviando POST a la API del Proyecto 1...');
  ...
  try {
    ...
    actualizarExplicacion(`Fetch obtuvo ${datos.length} alumno(s) de la API. Los datos vienen de MySQL a través del Proyecto 1.`);
  } catch (error) {
    actualizarExplicacion('No se pudo conectar con la API. Asegurate de que el Proyecto 1 esté activo en el puerto 3000.');
  }
}
...
async function cargarConAxios() {
  ...
  actualizarExplicacion('Axios es una librería que simplifica las peticiones HTTP. Enviando POST a la API del Proyecto 1...');
