...
const socialLogin = async (req, res) => {
    ...
    try {
        ...
        let user;
        if (users.length === 0) {
            // Register new social user as Alumno (Rol 3)
            const roleToAssign = 3;
            const [result] = await db.query('INSERT INTO users (name, email, password, role_id) VALUES (?, ?, ?, ?)', 
                [name, email, 'social_login', roleToAssign]);
...
module.exports = { register, login, socialLogin, getAllUsers, updateRole, deleteUser };
